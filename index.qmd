...
subtitle: "Étude comparative de 5 géants de la Tech"
author: "Eya Zantour"
date: today
format:
  html:
...
-   **Dataset**: 67,529 avis analysés
-   **Entreprises**: Amazon, Microsoft, Apple, Google, Facebook
-   **Période**: 2008-2023
-   **Note moyenne globale**: 3.83/5 ⭐
:::

**Objectif**: Analyser la satisfaction des employés dans les géants de la tech et identifier les facteurs clés de réussite et d'amélioration.
...
Les avis Glassdoor sont devenus un outil crucial pour les candidats évaluant les employeurs potentiels. Cette analyse examine les perceptions des employés actuels et anciens de 5 entreprises technologiques majeures.

## Questions de Recherche

1.  Quelle entreprise offre la meilleure expérience employé?
2.  Quelles sont les dimensions de satisfaction les plus critiques?
3.  Comment les notes évoluent-elles dans le temps?
4.  Quels sont les points forts et faibles spécifiques à chaque entreprise?
...
# Chargement des données avec toutes les colonnes
...
company_counts <- df %>%
  ...
  mutate(
    ...
    pourcentage_fmt = paste0(round(pourcentage, 1), "%")
  )

# Tableau interactif
datatable(
  company_counts,
  colnames = c("Entreprise", "Nombre d'avis", "Pourcentage", ""),
  options = list(pageLength = 5, dom = 't'),
  rownames = FALSE
)
```

```{r company-barplot, fig.cap="Répartition des avis par entreprise"}
company_counts %>%
...
La distribution est **légèrement asymétrique à gauche** avec une concentration autour de 4 étoiles, suggérant une satisfaction globalement positive mais avec une minorité significative d'avis très négatifs.
:::
...
df %>%
  ...
  labs(
    title = "Répartition des Avis par Sentiment",
    fill = "Catégorie"
  ) +
  theme_void() +
  theme(
...
company_stats %>%
  ...
  kable(
    ...
    col.names = c("Entreprise", "N Avis", "Moyenne", "Médiane", "Écart-type")
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
...
```{r statistical-tests}
# Test ANOVA pour différences significatives
...
cat("\n\nTest de Tukey (différences par paires):\n")
print(tukey_result$company[1:10, ])  # Afficher les 10 premières comparaisons
```

::: callout-warning
## ⚠️ Attention

Le test ANOVA révèle des **différences statistiquement significatives** entre les entreprises (p \< 0.001). Les différences observées ne sont pas dues au hasard.
:::
...
## Heatmap Entreprises × Dimensions
...
-   **Dimension la mieux notée**: Compensation & Benefits (moy. 3.94)

-   **Dimension la plus critique**: Senior Management (moy. 3.32)

    -   **Variabilité la plus forte**: Senior Management (écart-type 1.31)

-   **Facebook** excelle dans toutes les dimensions

-   **Amazon** et **Netflix** ont le plus de marge d'amélioration\
::::
...
```{r temporal-plot, fig.width=14, fig.height=8, fig.cap="Évolution temporelle des notes moyennes"}
...
```{r trend-analysis}
# Régression linéaire par entreprise
trend_models <- yearly_trends %>%
  group_by(company) %>%
  summarise(
    pente = coef(lm(note_moyenne ~ review_year))[2],
    tendance = ifelse(pente > 0, "↗️ Amélioration", "↘️ Dégradation"),
...
```{r}
# Créer une grille de 2x3 pour les 6 entreprises
...
## Mots-Clés dans les Points Positifs
...
pros_words %>%
  ...
  coord_flip() +
  labs(
    title = "Mots les Plus Fréquents dans les Points Positifs",
    x = NULL,
    y = "Fréquence"
  )
```

## Mots-Clés dans les Points Négatifs
...
```{r cons-barplot, fig.cap="Top 20 mots dans les points négatifs"}
cons_words %>%
  ggplot(aes(x = reorder(word, n), y = n)) +
  ...
  coord_flip() +
  labs(
    title = "Mots les Plus Fréquents dans les Points Négatifs",
    x = NULL,
    y = "Fréquence"
  )
```
...
cat("**Entreprise la mieux notée:**", best_company$company, "\n")
cat("- Note moyenne:", round(best_company$note_moyenne, 2), "/5\n")
cat("- Nombre d'avis:", format(best_company$n_avis, big.mark = ","), "\n")
cat("- Écart-type:", round(best_company$ecart_type, 2), "(forte cohérence)\n")
```
...
cat("**Entreprise avec le plus de marge d'amélioration:**", worst_company$company, "\n")
cat("- Note moyenne:", round(worst_company$note_moyenne, 2), "/5\n")
cat("- Nombre d'avis:", format(worst_company$n_avis, big.mark = ","), "\n")
cat("- Points d'amélioration prioritaires: Senior Management, Work-Life Balance\n")
```

### 📊 Dimension Critique

```{r weakest-dimension}
weakest_dim <- dimension_stats %>% slice_min(moyenne, n = 1)

cat("**Dimension la plus critique:**", weakest_dim$dimension, "\n")
cat("- Score moyen:", round(weakest_dim$moyenne, 2), "/5\n")
cat("- Impact: Tous les acteurs du secteur doivent investir dans cette dimension\n")
```
:::
...
2.  **Améliorer l'Équilibre Vie Pro/Perso**
    -   Flexibilité horaire
    -   Télétravail
    -   Congés sabbatiques
3.  **Clarifier les Opportunités de Carrière**
...
-   **Prioriser les dimensions importantes pour vous** plutôt que la note globale
-   **Lire les avis récents** (dernières 2-3 années)
-   **Analyser les tendances** d'amélioration/dégradation
-   **Considérer le contexte** (taille de l'équipe, localisation, département)

------------------------------------------------------------------------

# Conclusion

Cette analyse de 67,529 avis Glassdoor révèle des **différences significatives** entre les 5 géants de la tech étudiés. Bien que tous maintiennent des notes supérieures à 3.5/5, les écarts dans certaines dimensions (notamment le Senior Management et l'équilibre vie pro/perso) représentent des opportunités d'amélioration substantielles.

**Points clés à retenir:**

1.  La **culture d'entreprise** reste le point fort commun
2.  Le **management** est le défi universel du secteur
3.  Les **tendances temporelles** montrent une amélioration globale depuis 2015
4.  L'analyse textuelle confirme l'importance de la **flexibilité** et de l'**innovation**
...
-   Méthodologie: Analyse exploratoire des données (EDA)
-   Tests statistiques: ANOVA, Tukey HSD

## Code Source Complet

Le code source complet de cette analyse est disponible sur GitHub: \[lien-vers-repo\]
