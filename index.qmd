...
subtitle: "Étude comparative de 6 géants de la Tech"
author: "Eya Zantour"
date: today
format:
  html:
...
```` r
````r
```{r company-counts, echo=FALSE}
company_counts <- df %>%
  dplyr::count(company) %>%
  dplyr::mutate(
    pourcentage = n / sum(n),
    pourcentage_fmt = scales::percent(pourcentage, accuracy = 0.1)
  )
```
````
...
if(nrow(dimensions_long) > 0) {
  ...
  cat("⚠️ No valid dimension data available\n")
}
```
